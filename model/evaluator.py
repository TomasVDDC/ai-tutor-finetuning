...
def repository_check():
    ...
    assert os.path.isdir(
        "checkpoints"
    ), "You must have a directory named 'checkpoints' in the current directory."

    assert os.path.isdir(
        "models"
    ), "You must have a directory named 'models' in the current directory."

    files = [f for f in os.listdir(".") if os.path.isfile(f)]
    assert (
        "main_config.yaml" in files
    ), "main_config.yaml not found in the current directory."
    assert (
...
class DPOModelEvaluator:
    def __init__(
        ...
        if task_type == "causal_lm":
            ...
            raise ValueError(
                "Invalid task type! Please choose from 'causal_lm' or 'seq2seq'."
            )
    ...
    def load_tokenizer(self, model_path: str):
        ...
        try:
            ...
            logger.error(
                "Important! You must properly save the tokenizer in the model directory!"
            )
    ...
    def scoring_mcqa(self, test_dataloader: DataLoader):
        ...
        policy_model = self.model_class.from_pretrained(
            self.policy_model_path, **self.dpo_model_args
        )
        ...
        policy_accuracy = accuracy_score(y_true=all_labels, y_pred=all_policy_preds)
    ...
    def compute_reference_logprobs(self, test_data: List[Dict]):
        ...
        """
        assert (
            self.reference_model_path is not None
        ), "You must provide the path to the reference model for reward computation!"
        assert (
            self.reference_tokenizer is not None
        ), "You must provide the tokenizer for the reference model for reward computation!"
        ...
        args_pretrained = {"trust_remote_code": True}

        reference_model = self.model_class.from_pretrained(
            self.reference_model_path, **args_pretrained
        )

        for idx, batch in enumerate(test_dataloader):
            try:
                chosen_logps, rejected_logps = reference_model.get_logprobs(
                    batch, self.reference_tokenizer
                )
            except Exception as e:
                logger.error(
                    f"Error in batch {idx}: {e}! \n Please check your implementation and the return format!"
                )
                continue
            for prompt, chosen_logp, rejected_logp in zip(
                batch["prompt"], chosen_logps, rejected_logps
            ):
...
class RAGModelEvaluator:
    def __init__(self, rag_policy_model_path: str, rag_model_args: dict = {}):
        ...
        self.rag_dpo_evaluator = DPOModelEvaluator(
            policy_model_path=rag_policy_model_path, dpo_model_args=rag_model_args
        )
...
class QuantizedEvaluator:
    def __init__(
        ...
        ), "You must provide the path to the policy model!"
        assert (
            quantized_model_path is not None
        ), "You must provide the path to the quantized model!"
        ...
        self.quantized_dpo_evaluator = DPOModelEvaluator(
            policy_model_path=quantized_model_path, dpo_model_args=quantized_model_args
        )
...
if __name__ == "__main__":
    ...
    with open("main_config.yaml") as f:
        try:
            ...
            logger.error(
                f"Error loading main_config.yaml: {e}! Please check the file format."
            )
    ...
    assert not (
        "reward" in eval_method and "mcqa" in eval_method
    ), "You cannot evaluate both reward and mcqa at the same time!"
    ...
    if "reward" in eval_method:
        ...
        for method in eval_method:
            if method == "rag":
                ...
                assert os.path.isdir(
                    "documents"
                ), "You must have a directory named 'documents' with all the documents used for RAG, in the current directory."
                rag_policy_model_path = main_config["rag_policy_model_path"]
                evaluator = RAGModelEvaluator(rag_policy_model_path, rag_model_args)
                ...
                evaluator = QuantizedEvaluator(
                    policy_model_path, quantized_model_path, quantized_model_args
                )
                orig_size, quantized_size, quantized = (
                    evaluator.check_model_quantization()
                )
                metrics["orig_model_size"] = orig_size
                metrics["quantized_model_size"] = quantized_size
                if not quantized:
                    logger.error(
                        "Urgent! An error occurred that might result in 0 points!"
                    )
                    logger.error(
                        "Error: quantized model size should be less than the original model size!"
                    )
