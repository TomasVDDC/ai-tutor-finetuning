...
class AutoDPOModelForCausalLM(PreTrainedModelWrapper):
    ...
    def get_logprobs(self, batch, tokenizer):
        ...
        def calculate_logprobs(prompt_reponse_array):

            tokenizer.pad_token_id = tokenizer.eos_token_id
            tokenizer.pad_token = tokenizer.eos_token

            inputs = tokenizer(
                prompt_reponse_array,
                return_tensors="pt",
                padding = True,
                # padding="max_length",
                # max_length=256,
            ...
            outputs = outputs[:, :-1, :]  # remove the logits
            outputs_probs = F.log_softmax(outputs, dim=-1)  # remove the logits

            targets = inputs["input_ids"][:, 1:]  # remove the first token

            logps = torch.gather(
                outputs_probs, dim=2, index=targets.unsqueeze(2)
            ).squeeze(2)

            logps = (logps.detach() * inputs["attention_mask"][:,:-1]).sum(-1) #sum them up to get the probability of generating that sequence
            return logps
        #print(chosen_logps.shape)


        concat_chosen_arr = [
         prompt + chosen for prompt, chosen in zip(batch["prompt"], batch["chosen"])
        ]
        concat_rejected_arr = [
            prompt + rejected for prompt, rejected in zip(batch["prompt"], batch["rejected"])
         ]
 
        chosen_logps = calculate_logprobs(concat_chosen_arr)
        rejected_logps = calculate_logprobs(concat_rejected_arr)

        # # Same thing for rejected responses
        # concat_rejected_arr = [
        #     prompt + rejected for prompt, rejected in zip(batch["prompt"], batch["rejected"])
        # ]
        ...
        # rejected_targets = inputs["input_ids"][:, 1:]  # remove the first token

        # rejected_logps = torch.gather(
        #     outputs_probs, dim=2, index=rejected_targets.unsqueeze(2)
        # ).squeeze(2)
        # # print("rejected_logps.detach()" , rejected_logps.detach())
        # # print("inputs[attention_mask]" ,  inputs["attention_mask"])
        
        # rejected_logps = (rejected_logps.detach() * inputs["attention_mask"][:,:-1])
        # # print("rejected_logps" ,  rejected_logps)

        # rejected_logps_sum = rejected_logps.sum(-1)
        # # print("rejected_logps sum " ,  rejected_logps_sum)
    ...
    def prediction_step_reward(
        ...
        """
        output_dict = {"chosen_rewards": [], "rejected_rewards": []}
        ...
        # iterate over policy chosen logps
        for (
            new_policy_chosen_logp,
            new_policy_rejected_logp,
            new_reference_chosen_logp,
            new_reference_rejected_logp,
        ) in zip(
            policy_chosen_logps,
            ...
            chosen_reward = new_policy_chosen_logp - new_reference_chosen_logp
            rejected_reward = new_policy_rejected_logp - new_reference_rejected_logp
    ...
    def prediction_step_mcqa(self, batch, tokenizer):
        ...
        outputs = self.pretrained_model(**inputs) # [batch, prediction_class]
        print("logits", outputs.logits.shape)
        prediction_classes_for_all_logits = outputs.logits.argmax(dim=-1)
        print("predictions",prediction_classes_for_all_logits)

        prediction_letters = list(map(convert_to_letter, prediction_classes_for_all_logits))
